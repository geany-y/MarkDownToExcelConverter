...
1. **タスク2（基本構造解析）**: 現在実装中
   - 基本的なMarkdown構造の解析（見出し、段落、リスト、コードブロック等）
   - インデントレベルの検出
   - 行タイプの判定
   - **インライン書式記法は保持**（**太字**、*斜体*、~~取り消し線~~、`コード`、[リンク](URL)）
   - 除去要素の説明文化：
     - 引用: `> 引用文` → `引用：引用文`
     - 水平線: `---` → `水平線`
     - 表: `| 列1 | 列2 |` → `表：| 列1 | 列2 |`
...
2. **タスク3（書式解析）**: 今後実装予定
   - インライン書式記法の詳細解析
   - FormatInfo構造体への書式情報設定
   - 文字範囲での書式情報管理
   - Excel出力時の書式適用機能

**設計理由**
- 責任の明確な分離により、各タスクの複雑性を管理
- 基本構造が確立してから書式処理を実装することで、安定した基盤の上に構築
- テストとデバッグの容易性向上
