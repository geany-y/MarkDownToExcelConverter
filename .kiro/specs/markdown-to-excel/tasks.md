...
- [x] 2. Markdownパーサーの実装（基本構造解析）

**重要な実装方針**: タスク2では基本的な構造解析のみを行い、インライン書式記法（**太字**、*斜体*、~~取り消し線~~、`コード`、[リンク](URL)）は保持します。詳細な書式解析と記法除去はタスク3で実装します。

- [x] 2.1 基本的なMarkdown解析エンジンを実装
  - Markdownファイルの読み込み機能
  - 行ごとの基本解析（インデント検出、行タイプ判定）
  - DocumentLineオブジェクトへの変換
  - **インライン書式記法は保持**（タスク3で処理）
  - 除去要素の説明文化：引用（引用：）、水平線（水平線）、表（表：）
  - _要件: 1.1, 1.3, 2.1_

- [x] 2.2 プロパティテスト: ファイル読み込みと解析
  - **プロパティ1: ファイル読み込みと解析**
  - **検証対象: 要件 1.1, 1.3**
...
- [x] 2.4 プロパティテスト: インデントレベル検出
  - **プロパティ4: インデントレベル検出**
  - **検証対象: 要件 2.1**

- [x] 2.5 プロパティテスト: スペース・タブインデント認識
  - **プロパティ6: スペース・タブインデント認識**
  - **検証対象: 要件 2.4**

- [x] 2.6 Markdown要素の分類と変換を実装
  - 見出し、段落、リスト、コードブロックの識別
  - 表要素の除外処理
  - 各要素タイプの適切な処理
  - _要件: 3.1, 3.2, 3.3, 3.4, 3.5_

- [x] 2.7 プロパティテスト: 見出し要素変換
  - **プロパティ7: 見出し要素変換**
  - **検証対象: 要件 3.1**

- [x] 2.8 プロパティテスト: 段落テキスト変換
  - **プロパティ8: 段落テキスト変換**
  - **検証対象: 要件 3.2**

- [x] 2.9 プロパティテスト: リスト要素変換
  - **プロパティ9: リスト要素変換**
  - **検証対象: 要件 3.3**

- [x] 2.10 プロパティテスト: 表要素除外
  - **プロパティ10: 表要素除外**
  - **検証対象: 要件 3.4**

- [x] 2.11 プロパティテスト: コードブロック変換
  - **プロパティ11: コードブロック変換**
  - **検証対象: 要件 3.5**
